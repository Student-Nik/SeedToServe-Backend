...
public class AddressService {
	...
	public ResponseEntity<String> createAddress(AddressDetailsDTO addressDetailsDto, Customer customer) {

		Optional<AddressDetails> existing = addressRepository.findByMobileNo(addressDetailsDto.getMobileNo());

		if (existing.isPresent()) {
			return ResponseEntity.status(HttpStatus.CONFLICT)
					.body("The address for " + addressDetailsDto.getMobileNo() + " already exists!");
		}
	...
	public ResponseEntity<String> deleteAddress(int id) {
		...
		if (isExistingAddress.isPresent()) {
			...
			return ResponseEntity.status(HttpStatus.ACCEPTED).body("Address deleted Successfully!");
		} else {
	...
	@Transactional
	public ResponseEntity<String> updateAddress(AddressDetailsDTO addressDetailsDto, int id, Customer customer) {
		...
		if (isExistingAddress.isPresent()) {
			...
			return ResponseEntity.status(HttpStatus.OK).body("Address Details Updated Successfully!");
		}

		return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Address Details not found!");
	}
